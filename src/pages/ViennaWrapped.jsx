...
export default function WrappedStory({ onComplete }) {
  ...
  // De cruciale start-functie
  const handleStart = () => {
    // Forceer de browser om de media-context te openen
    if (audioRef.current) {
    ...
    if (videoRef.current) {
      ...
      // We starten en pauzeren direct om te 'bufferen' voor slide 11
  ...
  return (
    <div className="fixed inset-0 w-full h-full bg-black overflow-hidden touch-none select-none font-sans">
      ...
      {/* 1. MEDIA ELEMENTEN (ALTIJD AANWEZIG) */}
      ...
      {/* 3. STORY INTERFACE (ALLEEN ZICHTBAAR NA START) */}
      {started && (
        <>
          {/* Progress Bars */}
            ...
            >
              ...
              {s.layout === 'race' && (
                <div className="w-full h-full pt-28 pb-12 flex flex-col z-10 px-4">
                  ...
                  >
                    <h1 className="text-7xl italic leading-none mb-4 uppercase">Wenen!</h1>
                    <p className="bg-white text-black px-8 py-3 text-2xl uppercase rotate-3">GEWONNEN!</p>
                  </motion.div>
                </div>
              )}

              {/* VIDEO OVERLAY TEXT (Sam Says) */}
              {s.layout === 'video' && (
              ...
              {['hero', 'stats', 'genres', 'skip', 'fact', 'list', 'weird', 'music', 'age'].includes(s.layout) && s.layout !== 'mcd' && (
                <div className="z-10 flex flex-col items-center w-full px-4">
                  ...
                  {s.desc && <p className="text-xl font-bold text-white italic opacity-90 text-center mb-10 px-4 leading-tight">{s.desc}</p>}
                  {s.items && (
                    <div className="w-full space-y-3 px-4">
                      {s.items.map((it, idx) => (
                        <div key={idx} className="bg-white/10 border border-white/20 p-4 rounded-2xl font-black text-lg uppercase italic text-white shadow-lg">
                          {it}
                        </div>
                      ))}
                    </div>
